 ...
 * @returns {Quaternion}
 */
export function Quaternion(q0, q1, q2, q3) {
	if (q0 === undefined) {
		this.q = [1, 0, 0, 0];
	} else {
		this.q = [q0, q1, q2, q3];
...
Quaternion['fromEuler'] = function(psi, theta, gamma) {
	const s_psi   = Math.sin(psi*0.5);
	const c_psi   = Math.cos(psi*0.5);
	const s_theta = Math.sin(theta*0.5);
	const c_theta = Math.cos(theta*0.5);
	const s_gamma = Math.sin(gamma*0.5);
	const c_gamma = Math.cos(gamma*0.5);
